...
export default function BookOffice() {
  const { slug } = useParams<{ slug: string }>(); // ini adalah langkah untuk mengambil slug dari URL (Parameter dari URL yang saat ini aktif, lalu dimasukan ke dalam variable slug)
  ...
  const [totalAmmountWithUniqueCode, setTotalAmmountWithUniqueCode] =
    useState<number>(0);
  ...
  useEffect(() => {
    axios
      .get(`http://127.0.0.1:8000/api/office/${slug}`, {
        headers: {
          "X-API-KEY": "qwqq2332eqewew4klfe333r31f",
        },
      })
      .then((response) => {
        ...
        setTotalAmmountWithUniqueCode(grandTotal);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    console.log("Validatiing form data ...");
    ...
    try {
      const response = await axios.post(
        "http://127.0.1:8000/api/booking-transaction",
        {
          ...formData,
        },
        {
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-2">
              ...
              <div className="flex items-center rounded-full border border-[#000929] px-5 gap-[10px] transition-all duration-300 focus-within:ring-2 focus-within:ring-[#0D903A] overflow-hidden">
                ...
                {formErrors.some((error) =>
                  error.issues.some((issue) =>
                    issue.path.includes("started_at")
                  )
          ...
          <div className="flex flex-col gap-5">
            ...
            <div className="flex items-center justify-between">
              ...
              <p className="font-bold text-[22px] leading-[33px] text-[#0D903A]">
                Rp {totalAmmountWithUniqueCode.toLocaleString("id-ID")}
              </p>
            </div>
